...
export const getPrescriptionByRecord = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const prescription = await Prescription.findOne({ record_id: recordId }).populate("medicines.medicine_id");
...
export const updatePrescription = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const updated = await Prescription.findByIdAndUpdate(prescriptionId, { medicines }, { new: true });
